      ...
      >
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <a href="#" className="flex items-center gap-2.5 group" aria-label="Home">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-sm shadow-indigo-300 group-hover:scale-105 transition-transform">
          ...
          <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
            {[['Features','#features'],['Resources','#resources']].map(([l,h]) => (
              <a key={h} href={h} className="nav-link text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">{l}</a>
            ))}
          </nav>

          {/* Download CTA */}
          <a href="#download" id="nav-download"
          ...
          <motion.div key="mob"
            ...
            {[['Features','#features'],['Resources','#resources'],['Download','#download']].map(([l,h]) => (
              <a key={h} href={h} onClick={() => setMobileOpen(false)}
                className="text-base font-semibold text-slate-700 hover:text-indigo-600 transition-colors">{l}</a>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
